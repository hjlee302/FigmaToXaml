...
function addCommonProperties(target, node) {
    target.visible = node.visible;
    target.locked = node.locked;
    if ('x' in node)
        target.x = round(node.x);
    if ('y' in node)
        target.y = round(node.y);
    if ('width' in node)
        target.width = round(node.width);
    if ('height' in node)
        target.height = round(node.height);
    if ('rotation' in node)
        target.rotation = round(node.rotation);
    if ('opacity' in node)
        target.opacity = round(node.opacity);
}
function addLayoutProperties(target, node) {
    const source = node;
    const layout = {};
...
function addAppearanceProperties(target, node) {
    const source = node;
    const appearance = {};
...
function addTextProperties(target, node) {
    if (node.type !== 'TEXT') {
        return;
    }
    const text = {
...
function addVectorProperties(target, node) {
...
function serializeEffect(effect) {
    const result = serializePlainObject(effect);
...
function copyNumber(target, source, key) {
    const value = source[key];
    if (typeof value === 'number') {
        target[key] = round(value);
    }
}
function copyNumberArray(target, source, key) {
    const value = source[key];
...
function makeFileName(pageName, source) {
